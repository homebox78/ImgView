...
![우클릭 메뉴](docs/14-context.png)

> 여러 장을 선택한 상태에서 우클릭하면 메뉴 동작이 **선택한 전체에 일괄 적용**됩니다(예: 회전·압축·포맷변환·이름변경·삭제).
...
- **이름변경** — 한 장은 즉시 입력, **여러 장은 규칙으로 일괄 변경**합니다.
...
여러 장을 선택하고 이름변경(`F2`)을 누르면 규칙 기반 일괄 변경 창이 열립니다.

![여러 파일 이름 일괄 변경](docs/15-batch-rename.png)

- **이름 형식**에 변수를 섞어 씁니다 — `<번호>`·`<번호:3>`(자릿수) · `<날짜>` · `<시간>` · `<년>` · `<월>` · `<일>` · `<원본>`(원래 이름). 칩을 눌러 바로 삽입할 수 있습니다.
- **시작 번호 / 증가** 값으로 번호 매기기를 조절합니다.
- 아래 **미리보기**에서 `원래이름 → 새이름`을 실시간으로 확인한 뒤 `모두 변경`을 누릅니다.
